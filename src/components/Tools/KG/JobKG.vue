...
// 防止name冲突的函数
function addPrefixToNodes (nodes, prefix) {
  return nodes.map(node => ({
    ...node,
    name: `${prefix}${node.name}`
  }))
}
function addPrefixToLinks (links, prefix) {
...
function updateChartData (cvData, cardData) {
  if (cvData) {
    ...
    nodes.value = addPrefixToNodes(cvData.KG.nodes, 'CV_')
    links.value = addPrefixToLinks(cvData.KG.links, 'CV_')
    // 设置中心节点位置
    nodes.value.forEach((node) => {
      if (node.name === centerNode.value) {
  ...
  if (cardData) {
    ...
    nodes2.value = addPrefixToNodes(cardData.KG.nodes, 'Card_')
    links2.value = addPrefixToLinks(cardData.KG.links, 'Card_')
    // 设置中心节点位置
    nodes2.value.forEach((node) => {
      if (node.name === centerNode2.value) {
        node.fixed = true // 设置固定属性为true
...
const chartOption = computed(() => {
  return {
    ...
    legend: {
      ...
      left: 'center',
      icon: 'circle', // 圆形图例
      data: ['CV_' + centerNode.value, '期望', '教育', '证书', '经历', '项目', '技能', '经验', 'Card_' + centerNode2.value]
    },
    textStyle: {
    ...
    tooltip: {
      formatter: function (params) {
        ...
        if (params.data.name) {
          // 同样移除前缀，并显示节点的描述
          return `${params.data.name.replace(/^(CV_|Card_)/, '')}: ${params.data.description || ''}`
        }
        return ''
      }
    },
    series: [{
      ...
      label: {
        ...
        formatter: function (params) {
          // 检查name是否存在，如果不存在则返回空字符串
          if (params.data.name) {
            return params.data.name.replace(/^(CV_|Card_)/, '')
          }
          return ''
        }
      },
      ...
      categories: [
        ...
        { name: centerNode2.value, itemStyle: { color: getRadialGradient('rgba(233,153,197,0.5)', resources.subThemeColor), borderWidth: 1, borderColor: resources.subThemeColor } }
      ]
    }]
  }
})
